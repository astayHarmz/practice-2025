...
PATTERNS=("Line" "Text" "Special" "test" " " "\t")
FLAGS=("" "-i" "-v" "-c" "-l" "-n" "-s" "-e" "-o" "-h" 
       "-iv" "-ic" "-il" "-in" "-vc" "-vl" "-vn" "-lc" "-ln" "-nc")
VALGRIND_LOG="valgrind_grep_summary.log"
...
check_leaks() {
    ...
    echo "===== Valgrind check: grep $flag '$pattern' $file =====" >> "$VALGRIND_LOG"
    
    valgrind --tool=memcheck --leak-check=full --show-leak-kinds=all \
              --errors-for-leak-kinds=all --error-exitcode=1 \
              $command $flag "$pattern" "$file" >> "$VALGRIND_LOG" 2>&1
    
    if [ $? -ne 0 ]; then
        echo "MEMORY LEAK DETECTED: grep $flag '$pattern' $file" | tee -a "$VALGRIND_LOG"
        return 1
    else
    ...
    for file in "${TEST_FILES[@]}"; do
        for pattern in "${PATTERNS[@]}"; do
            for flag in "${FLAGS[@]}"; do
                ...
                if ! check_leaks "./grep/s21_grep" "$pattern" "$file" "$flag"; then
                    ((leaks++))
                fi
                
                original=$(grep $flag "$pattern" "$file" 2>&1)
                custom=$(./grep/s21_grep $flag "$pattern" "$file" 2>&1)

                if [ "$original" = "$custom" ]; then
                    ((passed++))
                else
                    ((failed++))
